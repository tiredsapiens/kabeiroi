...
static const char *memeory_tag_strings[MEMORY_TAG_MAX_TAGS] = {
...
void *kallocate(u64 size, memory_tag tag) {
  if (tag == MEMORY_TAG_UNKNOWN) {
    KWARN("kallocate called with unknown memory tag");
  }
...
void *kcopy_memory(void *dst, const void *source, u64 size) {
  return platform_copy_memory(dst, source, size);
}
...
char *get_memory_usage_str() {
  ...
  for (u32 i = 0; i < MEMORY_TAG_MAX_TAGS; i++) {
    ...
    i32 length=snprintf(buffer + offset, 8000, " %s: %.2f%s\n", memeory_tag_strings[i], amount, unit);
    offset += length;
  }
